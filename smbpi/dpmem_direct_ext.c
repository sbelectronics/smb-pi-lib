...
void dpmem_set_addr_nbits(unsigned int addr, unsigned int nbits)
{
  int i;
  for (i=0; i<nbits; i++) {
...
static PyObject *dpmem_direct_set_addr_nbits(PyObject *self, PyObject *args)
{
  int addr, nbits;
  if (!PyArg_ParseTuple(args, "ii", &addr, &nbits)) {
    return NULL;
  }
  dpmem_set_addr_nbits(addr, nbits);
  return Py_BuildValue("");
}
...
{
  unsigned int addr, nbits, data;
  if (!PyArg_ParseTuple(args, "iii", &addr, &nbits, &data)) {
    return NULL;
  }
  dpmem_config_output();
  dpmem_set_addr_nbits(addr, nbits);
...
{
  unsigned int addr, nbits, count;
  char buf[1024];
  int i;

  if (!PyArg_ParseTuple(args, "iii", &addr, &nbits, &count)) {
    return NULL;
  }
  ...
  for (i=0; i<count; i++) {
      dpmem_set_addr_nbits(addr + i, nbits);
      digitalWrite(DP_CE,0);
      digitalWrite(DP_R,0);
...
static PyObject *dpmem_direct_write_block(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "iis#i", &addr, &nbits, &buf, &buf_len, &count)) {
    return NULL;
  }
...
static PyMethodDef dpmem_direct_methods[] = {
  {"set_addr", dpmem_direct_set_addr, METH_VARARGS, "Set address bits with 10-bit address"},
  {"set_addr_nbits", dpmem_direct_set_addr_nbits, METH_VARARGS, "Set address bits with specified bit-length"},
